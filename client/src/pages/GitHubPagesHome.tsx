  ...
  return <div dir="rtl" className="min-h-screen bg-[#100b17] text-slate-100">
    ...
    <main className="mx-auto max-w-[1500px] space-y-6 px-4 py-6 sm:px-7 sm:py-9">
      <section className="search-hero overflow-hidden rounded-[28px] border border-fuchsia-200/10 bg-[#20102b] px-5 py-6 shadow-[0_24px_80px_rgba(46,24,61,0.3)] sm:px-9 sm:py-9">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-white/55"><span>מקורות:</span><span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">AGRON 2006</span><span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">Elector</span><span className="rounded-full border border-amber-200/15 bg-amber-200/5 px-3 py-1.5">Facebook · התאמה מועמדת</span></div>
      </section>
      ...
      {searched && !isSearching && <section className="space-y-4">
        ...
        {results.length > 0 ? <div className="grid gap-3 xl:grid-cols-2">{results.map((hit, index) => <article key={`${hit.source}-${hit.nationalId}-${index}`} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-fuchsia-200/70">מקור: {hit.source}</p><h3 className="mt-1 text-lg font-semibold">{hit.fullName}</h3></div><span className={`rounded-full px-3 py-1 text-xs ${hit.confidence === "exact-id" || hit.confidence === "phone-match" || hit.confidence === "facebook-id-match" ? "border border-emerald-200/20 bg-emerald-200/10 text-emerald-100" : "border border-amber-200/20 bg-amber-200/10 text-amber-100"}`}>{hit.confidence === "exact-id" ? "ת״ז בעמודת המקור" : hit.confidence === "phone-match" ? "טלפון מדויק" : hit.confidence === "facebook-id-match" ? "מזהה Facebook מדויק" : hit.confidence === "text-match" ? "התאמת טקסט" : "התאמה מועמדת — יש לאמת"}</span></div>
          <dl className="mt-4 grid gap-x-5 gap-y-2 text-sm sm:grid-cols-2"><div><dt className="text-xs text-white/45">תעודת זהות</dt><dd className="font-mono">{hit.nationalId || "לא נמצא"}</dd></div>{hit.facebookId && <div><dt className="text-xs text-white/45">מזהה Facebook</dt><dd className="font-mono">{hit.facebookId}</dd></div>}{hit.phone && <div><dt className="text-xs text-white/45">טלפון</dt><dd dir="auto">{hit.phone}</dd></div>}{hit.address && <div><dt className="text-xs text-white/45">כתובת</dt><dd>{hit.address}</dd></div>}{hit.city && <div><dt className="text-xs text-white/45">יישוב</dt><dd>{hit.city}</dd></div>}{hit.cityCode && <div><dt className="text-xs text-white/45">קוד יישוב</dt><dd>{hit.cityCode}</dd></div>}{hit.age && <div><dt className="text-xs text-white/45">גיל במקור</dt><dd>{hit.age}</dd></div>}{hit.birthDate && <div><dt className="text-xs text-white/45">תאריך לידה במקור</dt><dd>{hit.birthDate}</dd></div>}</dl>
