...
const workers = createWorkers({
  async import(job) {
    ...
    for await (const batch of batches(parseFile(job.data.filePath, job.data.format, job.data.sourceId, job.data.innerFormat), job.data.batchSize ?? 1000, { batchNumber: 0, lastProcessedRecord: 0, sourceId: job.data.sourceId })) {
      if (repository) {
